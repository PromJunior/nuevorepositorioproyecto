...
class PurchaseService:
    ...
    def create_purchase(db: Session, purchase_create: PurchaseCreate, user_id: int):
        ...
        supplier = (
            db.query(Supplier)
            .filter(Supplier.id == purchase_create.supplier_id)
            .first()
        )
        if not supplier:
            raise ValueError(
                f"Proveedor con id {purchase_create.supplier_id} no encontrado"
            )
        ...
        return purchase_crud.create_purchase_db_record(
